...
def revoke_egress(desc, port, cidrip, group_name, security_group):
...
def lambda_handler(event, context):
    ...
    for instance in instances:    
        ...
        for sg_id in all_sg_ids:
            ...
            ipse = list(security_group.ip_permissions_egress)
            
            for x in range(len(ipse)):
                
                IpRangesList = ipse[x]['IpRanges']
                    ...
                    if 'CidrIp' in iprs :
                        if 'Description' in iprs :
                            if any(['22' == str(ips[x]['FromPort']),'2299' == str(ips[x]['FromPort'])]):
                                ...
                                revoke_egress(iprs['Description'], str(ips[x]['FromPort']), iprs['CidrIp'], security_group.group_name, security_group)
                        else:
                            if any(['22' == str(ips[x]['FromPort']),'2299' == str(ips[x]['FromPort'])]):
